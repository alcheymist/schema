...
class mutall {
    ...
    //objects from Javascript. This method is called from export.php which is now index.php. 
...
class database extends schema {
    ...
    function __construct(
            ...
            //default is true
            public bool $throw_exception = true
    ) {
        //
    ...
    //
    //This is a function that does the retrieving of data for populating a form in either edit or normal mode.
    ...
    //query method for sqls that dont return a result.
    ...
    function get_friendly_part2(): \Generator {
        ...
        foreach ($this->columns as $col) {
            ...
            elseif ($col->is_id() && $col instanceof foreign) {
                yield from $col->away()->selector::get_friendly_part();
            }
        }
    }
...
class foreign extends capture implements ilink{
    ...
    function is_hierarchical(): bool {
        ...
        return
            ...
            && $this->ref->db_name == $this->entity->self::get_dbase()->name;
    }
}
...
class myerror implements expression, answer{
    ...
    public $msg;
    //
    //Create properties for all dynamic properties.
...
//
//This class supports reporting of errors from scripts.
abstract class component {
    ...
    public database $db;
    //
    //This is where we log all the errors raised by the execute methods of 
    //classes that are an extension of this one.
    public array $errors = [];
    //
    ...
    function __construct() {
        ...
        $this->db= new database("mutall_users");
    }
    //
    //Report all the errors raised by php scripts by logging them into the 
    //message table in the database.
    public function report_errors(array $errors): void{
        //
        // Compile the errors into a single message.
        $msg = implode("\n",$errors);
        //
        //Save the errors into the message table.
        //
        //Get the database name.
        $db = $this->dbase->name;
        //
        //1. Construct the questionnaire comprising of the data to save.
        $layouts = [
            //
            //Get the subject of the message to compile is errors.
            //(Identify the module causing the error).
            [$db, "msg", [], "subject", "Errors"],
            //
            //The message body.
            [$db, "msg", [], "text", $msg],
            //
            //The message date is the current date.
            [$db, "msg", [], "date", now()],
            //
            //All errors should be directed to mutal_data.
            [$db, "business", [], "id", "mutall_data"],
            //
            //The specific user is the data operater.
            [$db, "user", [], "name", "Francis Nyakundi"]
        ];
        //
