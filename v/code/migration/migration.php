<?php
//
//The namespace is the first declaration in a script before any other in a file.
...
class server{
    ...
    const CO = [
        'url'=>'https://mutall.co.ke/migration/mutallco_extractor.php',
        'username'=>'mutallco',
        'password' => ''
    ];
    //
    const DO = [
        'url'=> 'http://206.189.207.206/schema/v/code/migration/transmit.php',
        'username'=>'mutall',
        'password' => 'mutall'
    ];
    ...
    //Migrate data from a server to this local host
    function migrate(array /*{dbname, server}*/ $from):string /*as error*/{
        //
        ...
        $cmd = [
            'class'=>'mutall\migration\extractor',
            'cargs'=>[$from['dbname'], $from['server']],
            'method'=>'execute',
            'margs'=>[$sqls]
            ];
        //
        ...
        //current one
        $transmiter = new transmitter($from['server']['url'], $cmd);
        //
        //Use the sqls to extract data from remote and transmit it the current one
        $data/*Array<{tname, cnames, matrix}>|false*/ = $transmiter->execute();
        //
        ...
        $result /*:true|error as a string*/ = $ingester->execute();
        //
        return $result;
    }
    //
    ...
    private function get_sql($tname):array/*{tname: string, sql:string}*/{
        ...
        $sql = file_get_contents("../sqls/$tname.sql");
        //
...
class transmitter{
    ...
    */
    function __construct(string $server_url, array $cmd){
        //
        ...
        curl_setopt_array($this->curl, array(
            ...
            //
            //Set the url that links us to the remote server.
    ...
    */
    function execute():array|false/*Array<ingestee>|false*/{
        ...
        //
        //Check if the curl exceted successsfully. If not return a false
        //otherwise continue.
        if($ans === false) throw new \Error(`Curl failed to execute.`);
        //
        try {
            //
            //Json decode the answer to get a result that has the following structure.
            //{ok: boolean, result: string}
            $result = json_decode($ans,true , 512 ,\JSON_THROW_ON_ERROR) ;
            //
        } catch (\Exception $ex) {
            //
            //Throw the answer.
        ...
        //Check whether the extractor command was executed successfully or not.
        //if it was successful, we shall convert the results into an array of
        //ingestees and return it. otherwise we should report the error message
        //and discontinue.
        if(!$result['ok']) throw new \Error($result['result']);
        //
        //Convert the result of the html into an array of ingestees.
...
class ingester{
   ...
   //Execute use the ingester data to effect the ingestion process
   function execute():string /*as error*/{
       //
    ...
    function get_layout():\Generator{
        ...
        foreach($this->data as $ingester){
            ...
            yield from $this->get_table($tname, $columns, $matrix);
            //
    ...
    function get_table(string $tname,array /*<subject ={ename, cname}>*/ $columns, array/*matrix*/$matrix):\Generator {
        ...
        $table = new \stdClass();
        //
        $table->class_name = "\\mutall\\capture\\matrix";
        //
        $table->args = [
            //
            //Get the table name.
